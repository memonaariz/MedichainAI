...
def upsert_file_history_entry(new_entry):
    """Insert or replace a file history entry based on unique file_hash."""
    history = load_file_history()
    replaced = False
    for idx, entry in enumerate(history):
        if entry.get('file_hash') == new_entry.get('file_hash'):
            history[idx] = new_entry
            replaced = True
            break
    if not replaced:
        history.append(new_entry)
    # Keep most recent first if upload_time present
    try:
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            # Save to permanent file history (deduplicate by file_hash)
            upsert_file_history_entry({
                "filename": filename,
                "file_hash": file_hash,
...
def file_history():
    ...
    return render_template('file_history.html', file_history=deduped, chain=blockchain.chain)
